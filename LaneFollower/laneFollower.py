...
def motorControl(image, contours):
    if len(contours) > 0:
        ...
        cv2.line(image,(0, cy), (280, cy), (255, 0, 0), 1)

        cv2.drawContours(image, contours, -1, (0, 255, 0), 1)

        gpg.set_speed(100)
        gpg.steer(100 - 100 * math.cos((math.pi / 640) * cx), 100 - 100 * -math.cos((math.pi / 640) * cx))
        # if cx >= 340:
        #     gpg.left()
        # if 340 > cx > 300:
        #     gpg.forward()
...
with picamera.PiCamera() as camera:
    ...
    for frame in camera.capture_continuous(image, format="bgr", use_video_port=True):
        ...
        croppedImage = cropImage(lane_image, 200, 480, 0, 640)
